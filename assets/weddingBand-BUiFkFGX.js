import"./base-Dr4DB6T4.js";import{ap as X,e as L,i as Q,f as Y,l as J,j as Z,k as ee}from"./disposeScene-BTlNrOcD.js";import{l as te,c as G}from"./builderMaterials-BB6zQin4.js";import{c as ae}from"./inspector-CKoZaBMv.js";import{l as ne,a as ie,e as se,p as de,c as oe,h as re}from"./pricingService-DX4FvGoD.js";const le=[{size:4,diameterMm:14.07},{size:4.5,diameterMm:14.47},{size:5,diameterMm:14.88},{size:5.5,diameterMm:15.29},{size:6,diameterMm:15.69},{size:6.5,diameterMm:16.1},{size:7,diameterMm:16.51},{size:7.5,diameterMm:16.91},{size:8,diameterMm:17.32},{size:8.5,diameterMm:17.73},{size:9,diameterMm:18.13},{size:9.5,diameterMm:18.54},{size:10,diameterMm:18.95},{size:10.5,diameterMm:19.35},{size:11,diameterMm:19.76},{size:11.5,diameterMm:20.16},{size:12,diameterMm:20.57},{size:12.5,diameterMm:20.98},{size:13,diameterMm:21.38}],ce={width:{min:2,max:8,step:.5,default:4},thickness:{min:1.2,max:3,step:.1,default:1.8}},me={profile:"flat",comfortFit:!0,goldId:"yellow-gold",fingerSize:7},F={fingerSizes:le,ranges:ce,defaults:me},$=.001;function ue(e){return e*$/2}const pe=.08,ge=.25;function V(e){const{widthMm:d,thicknessMm:b,insideRadiusM:l,profile:E,comfortFit:p,opts:g={}}=e,c=Math.max(2,g.arcSegments??16),P=d*$,i=b*$,k=l,a=k+i,o=P/2,x=Math.min(o*.49,i*.4),h=(r=>Math.max(0,Math.min(r,x)))((g.edgeBevel??pe)*$),y=p?Math.max(0,Math.min((g.comfortDepth??ge)*$,i*.5,o*.8)):0,I=y>1e-6,w=I?0:h,C=I?k+y:k+w;let _=(g.domeHeight??.15*b)*$;_=Math.max(0,Math.min(_,i*.6));const s=E==="dome"&&_>1e-6,m=[],t=(r,f,M,v)=>{for(let u=0;u<=c;u++){const S=v+Math.PI/2*(u/c);m.push(new L(r+M*Math.cos(S),f+M*Math.sin(S)))}};if(m.push(new L(C,-o)),h>0?t(a-h,-o+h,h,-Math.PI/2):m.push(new L(a,-o)),s){const r=(o*o+_*_)/(2*_),f=a+_-r,M=Math.asin(o/r);for(let v=1;v<c;v++){const u=-M+2*M*v/c;m.push(new L(f+r*Math.cos(u),r*Math.sin(u)))}}if(h>0?t(a-h,o-h,h,0):m.push(new L(a,o)),m.push(new L(C,o)),I){const r=(y*y+o*o)/(2*y),f=k+r,M=Math.atan2(o,y-r),v=2*(Math.PI-M),u=Math.max(c,Math.ceil(c*v/(Math.PI/2)));for(let S=1;S<u;S++){const T=M+v*(S/u);m.push(new L(f+r*Math.cos(T),r*Math.sin(T)))}}else if(w>0){const r=k+w,f=o-w;for(let u=1;u<=c;u++){const S=Math.PI/2+Math.PI/2*(u/c);m.push(new L(r+w*Math.cos(S),f+w*Math.sin(S)))}m.push(new L(k,-o+w));const M=k+w,v=-o+w;for(let u=1;u<c;u++){const S=Math.PI+Math.PI/2*(u/c);m.push(new L(M+w*Math.cos(S),v+w*Math.sin(S)))}}return m.push(m[0].clone()),m}function fe(e,d=128){const b=V(e),l=new X(b,d);return l.computeVertexNormals(),l}function he(e,d){const{metals:b,karats:l,fingerSizes:E,ranges:p,defaults:g}=d,c=new Set(["yellow-gold","white-gold","rose-gold"]),P=n=>c.has(n),i=document.createElement("div");i.className="wedding-band-ui";const k=document.createElement("header");k.className="wedding-band-ui__header",k.innerHTML=`
    <a class="wedding-band-ui__back" href="../">&larr; Back</a>
    <span class="wedding-band-ui__title-strip">NOAM CARVER · Wedding Band</span>
  `,i.appendChild(k);const a=document.createElement("section");a.className="wedding-band-panel wedding-band-panel--left",a.innerHTML=`
