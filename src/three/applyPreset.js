...
export function applyPreset(material, preset, environments = null, options = 1) {
  ...
  // The `shader` preset field doesn't get applied here — it requires a
  // material class swap (MeshPhysicalMaterial -> ShaderMaterial) which is the
  // viewer's job. The inspector calls reapplyMeshMaterial() after
  // applyPreset() to handle that path.
